...
class QLearning:
    ...
    def learn(self):
        ...
        with torch.no_grad():
            ...
            max_next_q = next_q.max(dim=2, keepdim=True)[0]
            td_target = rewards + self.args.gamma * max_next_q * (1 - terminated)
    ...
    def _learn_from_episode_batch(self, batch):
        table = self.runner.controller.agent.table
        state_indices, action_indices, rewards, next_state_indices, terminated = self._build_transition_candidates(batch)
        ...
        with torch.no_grad():
            for idx in update_order:
                ...
                max_next_q = table[next_state_index].max().item()
                td_target = reward + self.args.gamma * max_next_q * (1 - is_terminated)
